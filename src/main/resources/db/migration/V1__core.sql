-- =============================================================================
-- V1__core.sql   (Flyway migration — derived from docs/sql/01_core.sql)
-- Pre-migration audit fixes folded in 2026-06-13. Search 'AUDIT-FIX' for each
-- applied change. Full rationale: docs/sql/PRE_MIGRATION_AUDIT.md;
-- decisions: docs/DECISION_LOG.md DL-BE-002..004.
...
CREATE TABLE deal_invoice (
    ...
    -- face_value stored in paise. positive_money_paise enforces > 0. (DL-016) [AUDIT-FIX: domain]
    face_value          positive_money_paise NOT NULL,
    ...
    -- [AUDIT-FIX] face_value > 0 now enforced by the positive_money_paise domain
...
CREATE TABLE deal_listing (
    ...
    buyer_limit_headroom_snapshot       money_paise,        -- paise [AUDIT-FIX: domain >=0]
    supplier_exposure_cap_snapshot      money_paise,        -- paise [AUDIT-FIX: domain >=0]
    funding_target                      positive_money_paise, -- paise; immutable after ready_for_review [AUDIT-FIX: domain >0]

    -- Funding state (DL-017, C12, G10)
    committed_total                     money_paise         NOT NULL DEFAULT 0,  -- [AUDIT-FIX: domain >=0]
    ...
    -- [AUDIT-FIX] funding_target > 0 / committed_total >= 0 now enforced by domains
    ...
    -- [AUDIT-FIX] a recorded GoneLive checker implies maker + MFA assertion present (L.4)
    CONSTRAINT deal_listing_golive_assertion_chk
        CHECK (golive_checker_id IS NULL
               OR (golive_maker_id IS NOT NULL AND golive_mfa_assertion_id IS NOT NULL)),
    -- [AUDIT-FIX #1] a closed listing always records its terminal_outcome, and a
    -- non-closed listing never carries one. Close(terminal_outcome) per B3 L.1 / line 114.
    CONSTRAINT deal_listing_terminal_outcome_shape_chk
        CHECK ((status = 'closed') = (terminal_outcome IS NOT NULL))
);
...
CREATE TABLE sub_subscription (
    ...
    -- amount in paise; positive_money_paise enforces > 0 (min 1,000,000 below). (DL-007, S.1) [AUDIT-FIX: domain]
    amount                          positive_money_paise        NOT NULL,
    status                          sub_subscription_status     NOT NULL DEFAULT 'committed',

    -- Inflow tracking (G10, C23, S.3)
    expected_inflow_amount          positive_money_paise        NOT NULL,  -- [AUDIT-FIX: domain >0]
    ...
    -- [AUDIT-FIX] amount > 0 / expected_inflow_amount > 0 now enforced by positive_money_paise
    ...
    CONSTRAINT sub_subscription_distribution_net_check
        CHECK (
            ...
            OR (
                -- [REVIEW-FIX #5] require all four money keys present, else a missing key makes
                -- ->> yield NULL and the whole CHECK evaluate to UNKNOWN (silently accepted).
                distribution_outcome ?& array['gross','tds','fee','net']
                AND (distribution_outcome->>'gross')::BIGINT
                  - (distribution_outcome->>'tds')::BIGINT
                  - (distribution_outcome->>'fee')::BIGINT
                  = (distribution_outcome->>'net')::BIGINT
                AND (distribution_outcome->>'gross')::BIGINT >= 0
                AND (distribution_outcome->>'tds')::BIGINT   >= 0
                AND (distribution_outcome->>'fee')::BIGINT   >= 0
                AND (distribution_outcome->>'net')::BIGINT   >= 0
            )
        )
);
...
--          (Rs 10 Cr) requires second_approver_id ≠ first_approver_id.
...
CREATE TABLE risk_buyer_profile (
    ...
    -- credit_limit in paise. positive_money_paise enforces > 0. (DL-022, BCP.1) [AUDIT-FIX: domain]
    credit_limit            positive_money_paise NOT NULL,
    ...
    -- [AUDIT-FIX] credit_limit > 0 now enforced by the positive_money_paise domain
    CONSTRAINT risk_buyer_profile_tenor_cap_range CHECK (tenor_cap_days BETWEEN 1 AND 180),
    -- [AUDIT-FIX] four-eyes columns are populated together (co-presence)
    CONSTRAINT risk_buyer_profile_four_eyes_pair_chk
        CHECK ((four_eyes_approval_ref IS NULL) = (second_approver_id IS NULL)),
    -- [AUDIT-FIX] four-eyes mandatory above Rs 10 Cr (C6, DL-023, BCP.2)
    CONSTRAINT risk_buyer_profile_four_eyes_required_chk
        CHECK (credit_limit <= 10000000000
               OR (four_eyes_approval_ref IS NOT NULL AND second_approver_id IS NOT NULL))
);
...
COMMENT ON COLUMN risk_buyer_profile.four_eyes_approval_ref IS
    'Reference to the FourEyesApproval envelope when credit_limit > Rs 10 Cr '
...
COMMENT ON COLUMN risk_buyer_profile.second_approver_id IS
    ...
    'Required when credit_limit > 10,000,000,000 paise (Rs 10 Cr). '
...
--          (Rs 10 Cr) requires second_approver_id ≠ first_approver_id. (C6, DL-023)
--          App layer MUST refuse if second_approver_id IS NULL when
...
CREATE TABLE risk_supplier_profile (
    ...
    -- exposure_cap in paise. money_paise enforces >= 0. (DL-022, SCP.1) [AUDIT-FIX: domain]
    exposure_cap            money_paise NOT NULL,
    conditions              JSONB       NOT NULL DEFAULT '[]',
    last_review_at          TIMESTAMPTZ,
    ...
    -- [AUDIT-FIX] exposure_cap >= 0 now enforced by the money_paise domain
    CONSTRAINT risk_supplier_profile_four_eyes_pair_chk
        CHECK ((four_eyes_approval_ref IS NULL) = (second_approver_id IS NULL)),
    -- [AUDIT-FIX] four-eyes mandatory above Rs 10 Cr (C6, DL-023, SCP.2)
    CONSTRAINT risk_supplier_profile_four_eyes_required_chk
        CHECK (exposure_cap <= 10000000000
               OR (four_eyes_approval_ref IS NOT NULL AND second_approver_id IS NOT NULL))
);

COMMENT ON TABLE risk_supplier_profile IS
    'BC3 SupplierCreditProfile aggregate. Risk rating and exposure cap for a '
    'supplier. Four-eyes required when cap > Rs 10 Cr. (DL-022, DL-023, C6)';
...
COMMENT ON COLUMN risk_supplier_profile.second_approver_id IS
    'Required when exposure_cap > 10,000,000,000 paise (Rs 10 Cr). '
...
CREATE TABLE risk_pricing_policy (
    ...
    -- Rate range in basis points (bps_type: 0..100000). min > 0; min ≤ max. (DL-024, PB.1) [AUDIT-FIX: domain]
    rate_range_min_bps      bps_type            NOT NULL,
    rate_range_max_bps      bps_type            NOT NULL,
    -- Platform fee in basis points (bps_type: 0..100000). (DL-024, PB.1) [AUDIT-FIX: domain]
    fee_bps                 bps_type            NOT NULL,
    ...
    -- [AUDIT-FIX] fee_bps >= 0 and the 0..100000 ceiling now enforced by bps_type
);
...
-- [AUDIT-FIX] index the self-FK supersession chain (was unindexed)
CREATE INDEX idx_risk_pricing_policy_superseded_by ON risk_pricing_policy (superseded_by)
...
--   DC.2  If the listing's outstanding exposure > 10,000,000,000 paise (Rs 10 Cr),
...
CREATE TABLE risk_default_case (
    ...
    -- Four-eyes approval reference when exposure > Rs 10 Cr. (C6, DL-023, DC.2)
    four_eyes_approval_ref  TEXT,
    second_approver_id      UUID,
    ...
    maker_id                UUID,
    checker_id              UUID,
    -- [AUDIT-FIX #3] MFA assertion behind the Classify decision (C7); mirrors
    ...
    -- [AUDIT-FIX] four-eyes columns are populated together (co-presence)
    CONSTRAINT risk_default_case_four_eyes_pair_chk
        CHECK ((four_eyes_approval_ref IS NULL) = (second_approver_id IS NULL)),
    -- [REVIEW-FIX #6/DL-BE-011] classified⇔outcome lifecycle is app-owned for now, so the
    -- maker's ProposeClassification step can persist a proposed outcome before status flips.
    -- [AUDIT-FIX #3] an approving checker implies a recorded MFA assertion (C7)
    CONSTRAINT risk_default_case_checker_mfa_chk
        CHECK (checker_id IS NULL OR checker_mfa_assertion_id IS NOT NULL)
);
...
COMMENT ON COLUMN risk_default_case.four_eyes_approval_ref IS
    'Required when listing exposure > 10,000,000,000 paise (Rs 10 Cr). '
...
COMMENT ON COLUMN risk_default_case.checker_mfa_assertion_id IS
    'MFA assertion ref for checker_id at Classify time. Records the fresh-MFA session '
    'behind a default/fraud classification, the platform''s most sensitive money decision. '
    '(C7, AUDIT-FIX #3)';
...
-- [AUDIT-FIX] index the self-FK re-classification chain (was unindexed)
CREATE INDEX idx_risk_default_case_corrects ON risk_default_case (corrects_case_id)
    WHERE corrects_case_id IS NOT NULL;
...
CREATE TABLE cash_virtual_account (
    ...
    -- Running inflow ledger in paise. money_paise enforces >= 0. (V.2, C23, G6) [AUDIT-FIX: domain]
    expected_inflow_total       money_paise                 NOT NULL DEFAULT 0,
    observed_inflow_total       money_paise                 NOT NULL DEFAULT 0,
    ...
    -- [AUDIT-FIX] inflow totals >= 0 now enforced by the money_paise domain
    -- [REVIEW-FIX #7/DL-BE-011] VA bank-coordinate/status lifecycle is app-owned for now:
    -- the bank's close webhook may set closed_at_bank after the status flips to 'closed'.
);
...
CREATE TABLE cash_payout_instruction (
    ...
    gross_amount                positive_money_paise    NOT NULL,  -- [AUDIT-FIX: domain >0]
    net_amount                  positive_money_paise    NOT NULL,  -- [AUDIT-FIX: domain >0]
    fee_amount                  money_paise             NOT NULL,  -- [AUDIT-FIX: domain >=0]
    total_tds_amount            money_paise,                       -- [AUDIT-FIX: domain >=0]
    ...
    -- [AUDIT-FIX] gross>0 / net>0 / fee>=0 / tds>=0 now enforced by money domains
    -- [AUDIT-FIX FN1] kind discriminates the owning aggregate (PI.2 / PI.4)
    CONSTRAINT cash_payout_instruction_kind_target_chk
        CHECK ((kind IN ('disbursement','distribution')
                AND listing_id IS NOT NULL AND subscription_id IS NULL)
               OR (kind = 'refund'
                AND subscription_id IS NOT NULL AND listing_id IS NULL)),
    -- [AUDIT-FIX] an approving checker implies a recorded MFA assertion (PI.5)
    CONSTRAINT cash_payout_instruction_checker_mfa_chk
        CHECK (checker_id IS NULL OR checker_mfa_assertion_id IS NOT NULL)
);
...
-- [AUDIT-FIX] index maker_id for treasury / maker-checker audit lookups
CREATE INDEX idx_cash_payout_instruction_maker        ON cash_payout_instruction (maker_id);
...
CREATE TABLE cash_remediation_case (
    ...
    -- [AUDIT-FIX] 'resolved' requires the corrective event + resolution doc (RC.1)
    CONSTRAINT cash_remediation_case_resolved_shape_chk
        CHECK (status <> 'resolved'
               OR (linked_corrective_event_id IS NOT NULL AND resolution_doc_hash IS NOT NULL))
);
...
CREATE TABLE legal_master_agreement (
    ...
    -- [AUDIT-FIX] cert/stamp/failed columns gated by status (MA.3)
    CONSTRAINT legal_master_agreement_status_shape_chk
        CHECK ((status IN ('initiated','failed') OR signature_cert_serial IS NOT NULL)
               AND (status = 'stamped' OR stamp_cert_id IS NULL)
               AND (status = 'failed'  OR failed_reason IS NULL))
);
...
-- [AUDIT-FIX] at most one active (signed/stamped) agreement per (party_id, kind) (MA.1)
CREATE UNIQUE INDEX uidx_legal_master_agreement_active_party_kind
    ON legal_master_agreement (party_id, kind)
    WHERE status IN ('signed','stamped');
...
COMMENT ON COLUMN legal_assignment_set.legs IS
    ...
    '[AUDIT-FIX #4] Money-conservation (sum of allocation_paise = listing committed_total) is '
    'a cross-aggregate APP-LAYER invariant with a mandatory invariant test (DoD); not a single-row CHECK.';
...
CREATE TABLE legal_signature_request (
    ...
    CONSTRAINT legal_signature_request_retry_nonneg
        CHECK (retry_count >= 0),
    -- [AUDIT-FIX] cert_serial only on terminal 'completed' (SR.3)
    CONSTRAINT legal_signature_request_cert_only_on_completed_chk
...
CREATE TABLE col_maturity_case (
    ...
    -- Running recovery total in paise. money_paise enforces >= 0. (MC.5, DL-029) [AUDIT-FIX: domain]
    recovery_total              money_paise         NOT NULL DEFAULT 0,
    ...
    -- [AUDIT-FIX] recovery_total >= 0 now enforced by the money_paise domain
    -- [REVIEW-FIX #4/DL-BE-011] outcome/delay_status lifecycle is app-owned for now: the BC3
    -- DefaultCase.Classified subscriber sets outcome while delay_status is 'under_adjudication'.
);
...
CREATE TABLE col_claim_case (
    ...
    outcome                     col_maturity_outcome,
    -- Supplier indemnity amount in paise, money_paise (>= 0); dilution/fraud only. (CCL.3) [AUDIT-FIX: domain]
    supplier_indemnity_amount   money_paise,
    ...
    CONSTRAINT col_claim_case_maturity_fk
        FOREIGN KEY (maturity_case_id) REFERENCES col_maturity_case (maturity_case_id),
    -- [AUDIT-FIX] indemnity >= 0 now enforced by the money_paise domain
    -- [AUDIT-FIX] outcome only when resolved (CCL.2); indemnity only for dilution/fraud (CCL.3)
    CONSTRAINT col_claim_case_outcome_shape_chk
        CHECK (outcome IS NULL OR status = 'resolved'),
    CONSTRAINT col_claim_case_indemnity_kind_chk
        CHECK (supplier_indemnity_amount IS NULL OR claim_type IN ('dilution','fraud'))
);
