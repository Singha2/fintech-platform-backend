-- =============================================================================
-- V3__auth.sql  (Flyway migration — derived from docs/sql/03_auth.sql)
-- Pre-migration audit fixes applied 2026-06-13: closes the two auth-layer gaps the
-- completeness critic flagged (assertion_id uniqueness/index; unfiltered FK-coverage
-- indexes). See docs/sql/PRE_MIGRATION_AUDIT.md, DECISION_LOG.md DL-BE-008. Search 'AUDIT-FIX'.
...
-- Closes the auth-layer gaps the audit's completeness critic flagged (this file
-- was the least-audited area). Declarative-only, no triggers (DL-BE-002).
-- =============================================================================

-- [AUDIT-FIX] The mfa_assertion_id anchor must be globally unique and indexed.
-- auth_session.mfa_assertion_id and every admin audit envelope's actor.mfa_assertion_id
-- reference auth_otp_challenge.assertion_id; the MFA-freshness join (invariant A3) needs
-- it unambiguous and fast. Partial (WHERE NOT NULL) since only login_mfa challenges mint one.
CREATE UNIQUE INDEX uidx_auth_otp_challenge_assertion
    ON auth_otp_challenge (assertion_id)
    WHERE assertion_id IS NOT NULL;

-- [AUDIT-FIX] Unfiltered FK-coverage indexes on identity_id. The existing identity_id
-- indexes are PARTIAL (WHERE revoked_at IS NULL / status = 'active'), which leaves the
-- ON DELETE RESTRICT integrity check and full-history lookups (incl. revoked/expired rows,
-- e.g. anomaly review / audit) without index support.
CREATE INDEX idx_auth_credential_identity_all    ON auth_credential (identity_id);
CREATE INDEX idx_auth_mfa_factor_identity_all     ON auth_mfa_factor (identity_id);
CREATE INDEX idx_auth_otp_challenge_identity_all  ON auth_otp_challenge (identity_id);
CREATE INDEX idx_auth_session_identity_all        ON auth_session (identity_id);
