-- =============================================================================
-- V4__generic_acl.sql  (Flyway migration — derived from docs/sql/04_generic_acl.sql)
-- Pre-migration audit fixes applied 2026-06-13: command-idempotency table + audit
-- tamper-evidence (DL-BE-003) and declarative hardening. The append-only TRIGGER is
-- intentionally KEPT here — its trigger->WORM-REVOKE replacement is the V4 decision pass
-- (effective REVOKE needs a non-owner app role). See PRE_MIGRATION_AUDIT.md, DL-BE-009.
...
CREATE TABLE gate_inflow_observation (
    ...
    amount                  positive_money_paise    NOT NULL,  -- [AUDIT-FIX: domain >0]
    ...
    -- [AUDIT-FIX] amount > 0 now enforced by the positive_money_paise domain
);
...
-- Fix-now items from DL-BE-003 (command idempotency + audit tamper-evidence) plus
-- declarative hardening. NOTE: the sys_audit_event append-only TRIGGER above is
-- intentionally LEFT IN PLACE — replacing it with a WORM REVOKE/GRANT depends on
-- production role topology (REVOKE is ineffective while the app owns the table), so
-- that swap is the V4 decision pass (DL-BE-009). No new triggers are added here.
-- =============================================================================

-- [AUDIT-FIX FN2] Producer-dedup table for command_id idempotency (non-negotiable #4, G18).
-- Handlers INSERT ... ON CONFLICT DO NOTHING and treat a conflict as a replay (no-op).
CREATE TABLE sys_command_log (
    ...
    CONSTRAINT sys_command_log_pk PRIMARY KEY (actor_id, command_id)
);
COMMENT ON TABLE sys_command_log IS
    'Producer-side command de-duplication (non-negotiable #4, G18). PK (actor_id, command_id) makes '
    'replaying a command a no-op. Reconcile with the existing vendor-side idempotency keys '
    '(cash_payout_instruction.payout_instruction_id, legal_signature_request idempotency uidx, '
    'gate_vendor_instruction.vendor_event_id, gate_inflow_observation.utr/vendor_event_id) so '
    'idempotency is not double-implemented (DL-BE-003).';
CREATE INDEX idx_sys_command_log_aggregate ON sys_command_log (aggregate_type, aggregate_id);

-- [AUDIT-FIX FN3] sys_audit_event tamper-evidence: the self-hash was absent, so the chain could
-- not be verified. Add envelope_hash (app-computed SHA-256 over the canonical row minus
-- envelope_hash) + length guards. Add command_id (idempotency key recorded on the envelope;
-- NULL for scheduler/webhook-originated events per B2). Plus AE.5/C7 actor guards.
ALTER TABLE sys_audit_event
    ADD COLUMN envelope_hash BYTEA NOT NULL,
    ADD COLUMN command_id    UUID;
ALTER TABLE sys_audit_event
    ADD CONSTRAINT sys_audit_event_envelope_hash_len
        CHECK (octet_length(envelope_hash) = 32),
    ADD CONSTRAINT sys_audit_event_prev_hash_len
        CHECK (previous_envelope_hash IS NULL OR octet_length(previous_envelope_hash) = 32),
    ADD CONSTRAINT sys_audit_event_actor_keys
        CHECK (actor ? 'actor_type' AND actor ? 'actor_id' AND actor ? 'session_id'),
    -- [REVIEW-FIX #2/DL-BE-011] removed the blanket admin-MFA CHECK. sys_audit_event records ALL
    -- admin activity — incl. login-success, the MFA-challenge events that *produce* the assertion,
    -- and sensitive reads — which legitimately carry no mfa_assertion_id. Enforcing it here rejected
    -- those valid appends. MFA-fresh on state-changing commands stays per-aggregate + app-side (C7).
    ADD CONSTRAINT sys_audit_event_time_order
        CHECK (occurred_at <= recorded_at);
CREATE UNIQUE INDEX uidx_sys_audit_event_envelope_hash ON sys_audit_event (envelope_hash);

-- [AUDIT-FIX] BC17 verification: failure_class present iff the row is in a failure state (V.6).
ALTER TABLE gate_verification
    ADD CONSTRAINT gate_verification_failure_class_shape_chk
        CHECK ((status IN ('failed','manual_fallback') AND failure_class IS NOT NULL)
            OR (status NOT IN ('failed','manual_fallback') AND failure_class IS NULL));

-- [AUDIT-FIX] BC15 notifications: ND.4 retry_count had no home; provider_ref must exist once sent.
ALTER TABLE sys_notification_dispatch
    ADD COLUMN retry_count SMALLINT NOT NULL DEFAULT 0;
ALTER TABLE sys_notification_dispatch
    ADD CONSTRAINT sys_notification_dispatch_retry_nonneg
        CHECK (retry_count >= 0),
    ADD CONSTRAINT sys_notification_dispatch_provider_ref_shape_chk
        CHECK (status NOT IN ('sent','delivered') OR provider_ref IS NOT NULL);

-- [AUDIT-FIX] BC19 signing: doc-centric lookup (all signing sessions for a document).
CREATE INDEX idx_vsr_doc_hash ON gate_signature_session (doc_hash);


-- =============================================================================
-- AUDIT-FIX DECISION PASS (DL-BE-010, 2026-06-13)
-- (A) The append-only TRIGGER is KEPT as a documented exception to the no-trigger
--     rule: audit immutability is a hard compliance control and REVOKE is ineffective
--     until the app connects as a restricted (non-owner) role. Revisit at that deploy.
-- (B) explicit chain shard key. (C) state-transition snapshot enforcement.
-- (D) banking-status enum superset + HMAC column naming consistency.
-- =============================================================================

-- (B) Explicit per-shard chain key (G25): makes 'first row per shard' and chain
-- verification robust/queryable instead of app-inferred. App populates as '<context>:<business_date>'.
ALTER TABLE sys_audit_event ADD COLUMN chain_shard TEXT NOT NULL;
DROP INDEX idx_audit_chain_verify;
CREATE INDEX idx_audit_chain_shard_verify
    ON sys_audit_event (chain_shard, recorded_at, event_id);
COMMENT ON COLUMN sys_audit_event.chain_shard IS
    'Explicit hash-chain shard key (G25). previous_envelope_hash is NULL only for the first row '
    'within a chain_shard. App-populated, e.g. <context>:<business_date>.';

-- (C) State-transition events must carry before/after snapshots (AE).
ALTER TABLE sys_audit_event ADD COLUMN is_state_transition BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE sys_audit_event ADD CONSTRAINT sys_audit_event_transition_snapshot_chk
    CHECK (NOT is_state_transition OR (before_state IS NOT NULL AND after_state IS NOT NULL));

-- (D) Extend banking-instruction status to the B3 superset (additive — existing values kept),
-- and align the BC17 HMAC-timestamp column name with BC18/BC19 (hmac_verified_at).
ALTER TYPE vendor_instruction_status_enum ADD VALUE IF NOT EXISTS 'acknowledged';
ALTER TYPE vendor_instruction_status_enum ADD VALUE IF NOT EXISTS 'webhook_received';
ALTER TYPE vendor_instruction_status_enum ADD VALUE IF NOT EXISTS 'reconciled';
ALTER TABLE gate_verification RENAME COLUMN signature_verified_at TO hmac_verified_at;
