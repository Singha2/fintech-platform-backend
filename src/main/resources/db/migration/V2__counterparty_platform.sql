-- =============================================================================
-- V2__counterparty_platform.sql  (Flyway migration — derived from docs/sql/02_counterparty_platform.sql)
-- Pre-migration audit fixes applied 2026-06-13. Mechanical declarative fixes only; design-level
-- items (maker-checker columns, SAR lifecycle split, enum naming) deferred to the V2 decision pass.
-- See docs/sql/PRE_MIGRATION_AUDIT.md and DECISION_LOG.md DL-BE-006. Search 'AUDIT-FIX'.
...
-- [AUDIT-FIX DP] invite status promoted from TEXT+CHECK to a typed enum.
CREATE TYPE inv_invite_status AS ENUM ('pending', 'consumed', 'expired');
...
-- [AUDIT-FIX DP] supplier financial-profile status promoted from TEXT+CHECK to a typed enum.
CREATE TYPE sup_financial_profile_status AS ENUM ('submitted', 'reviewed');
...
CREATE TYPE comp_kyc_file_status AS ENUM (
    'submitted',   -- [AUDIT-FIX DP] was 'in_review'; aligned to spec B3 KycFile state
    'approved',
    'rejected'
);
...
CREATE TABLE inv_invite (
    ...
    status              inv_invite_status NOT NULL DEFAULT 'pending',  -- [AUDIT-FIX DP] enum
...
CREATE TABLE sup_financial_profile (
    ...
    status                      sup_financial_profile_status NOT NULL DEFAULT 'submitted',  -- [AUDIT-FIX DP] enum
...
CREATE TABLE comp_kyc_file (
    ...
    status              comp_kyc_file_status    NOT NULL DEFAULT 'submitted',  -- [AUDIT-FIX DP]
...
CREATE INDEX idx_comp_kyc_file_status  ON comp_kyc_file (status) WHERE status = 'submitted';
...
CREATE TABLE tax_year_profile (
    ...
    tds_rate_bps                bps_type    NOT NULL DEFAULT 0,  -- [AUDIT-FIX] bps_type domain (0..100000) replaces raw INT + nonneg CHECK
...
-- =============================================================================
-- AUDIT-FIX SECTION (pre-migration audit, 2026-06-13)
-- Declarative-only hardening: discriminator-gated nullability CHECKs, value-set
-- CHECKs, and FK / natural-key index coverage. No procedural triggers (DL-BE-002).
-- Added as ALTER/CREATE after the deferred-FK block so every column already exists.
-- Design-level items (record-level maker-checker columns for Suspend/Blacklist/KYC,
-- SAR lifecycle split, comp_kyc_file 'in_review'->'submitted', TEXT->enum promotions)
-- are NOT here — they are the V2 decision pass.
-- =============================================================================

-- --- conditional-nullability / co-presence CHECKs -----------------------------------------
-- form-16A companion columns present together iff issued
ALTER TABLE tax_year_profile ADD CONSTRAINT tax_year_profile_form16a_shape_chk
    CHECK ((form_16a_issued = TRUE  AND form_16a_doc_hash IS NOT NULL AND form_16a_issued_at IS NOT NULL)
        OR (form_16a_issued = FALSE AND form_16a_doc_hash IS NULL     AND form_16a_issued_at IS NULL));

-- supplier suspend/blacklist reason travels with its timestamp
ALTER TABLE sup_account ADD CONSTRAINT sup_account_suspend_shape_chk
    CHECK ((suspended_at IS NULL) = (suspension_reason IS NULL));
ALTER TABLE sup_account ADD CONSTRAINT sup_account_blacklist_shape_chk
    CHECK ((blacklisted_at IS NULL) = (blacklist_reason IS NULL));

-- investor / buyer suspend reason travels with its timestamp
ALTER TABLE inv_account ADD CONSTRAINT inv_account_suspend_shape_chk
    CHECK ((suspended_at IS NULL) = (suspension_reason IS NULL));
ALTER TABLE buyer_account ADD CONSTRAINT buyer_account_suspend_shape_chk
    CHECK ((suspended_at IS NULL) = (suspension_reason IS NULL));

-- auditor account lifecycle timestamps gated by status
ALTER TABLE audit_account ADD CONSTRAINT audit_account_lifecycle_shape_chk
    CHECK ((status <> 'activated' OR activated_at IS NOT NULL)
       AND (auto_disabled_at IS NULL OR status = 'auto_disabled')
       AND (status <> 'auto_disabled' OR auto_disabled_at IS NOT NULL));

-- KYC refresh completion timestamp only when completed
ALTER TABLE comp_refresh_schedule ADD CONSTRAINT comp_refresh_schedule_completed_shape_chk
    CHECK (status = 'completed' OR completed_at IS NULL);

-- --- AML adjudication: gate the decision value + constrain it to the known set ------------
ALTER TABLE comp_aml_screening DROP CONSTRAINT comp_aml_screening_adjudication_chk;
ALTER TABLE comp_aml_screening ADD CONSTRAINT comp_aml_screening_adjudication_chk
    CHECK ((status = 'adjudicated' AND adjudication_decision IS NOT NULL
                AND adjudicated_by IS NOT NULL AND adjudicated_at IS NOT NULL)
        OR (status <> 'adjudicated' AND adjudication_decision IS NULL
                AND adjudicated_by IS NULL AND adjudicated_at IS NULL));
ALTER TABLE comp_aml_screening ADD CONSTRAINT comp_aml_screening_decision_values_chk
    CHECK (adjudication_decision IS NULL
           OR adjudication_decision IN ('clear','false_positive','true_hit_suspend'));

-- --- FK index coverage (Postgres does not auto-index FK columns) --------------------------
CREATE INDEX idx_buyer_payment_rule_superseded_by ON buyer_payment_rule (superseded_by) WHERE superseded_by IS NOT NULL;
CREATE INDEX idx_comp_kyc_file_approver           ON comp_kyc_file (approver_id) WHERE approver_id IS NOT NULL;
CREATE INDEX idx_tax_tds_deduction_listing        ON tax_tds_deduction (listing_id);
CREATE INDEX idx_inv_account_kyc_approved_by      ON inv_account (kyc_approved_by) WHERE kyc_approved_by IS NOT NULL;
CREATE INDEX idx_sup_account_kyc_approved_by      ON sup_account (kyc_approved_by) WHERE kyc_approved_by IS NOT NULL;
CREATE INDEX idx_sup_agency_consent_revoked_by    ON sup_agency_consent (revoked_by) WHERE revoked_by IS NOT NULL;
CREATE INDEX idx_buyer_account_nominated_by       ON buyer_account (nominated_by);
CREATE INDEX idx_buyer_ack_user_designated_by     ON buyer_ack_user (designated_by);
CREATE INDEX idx_buyer_payment_rule_confirmed_by  ON buyer_payment_rule (confirmed_by);
CREATE INDEX idx_admin_user_disabled_by           ON admin_user (disabled_by) WHERE disabled_by IS NOT NULL;
CREATE INDEX idx_admin_role_assignment_assigned_by ON admin_role_assignment (assigned_by);
CREATE INDEX idx_admin_role_assignment_revoked_by  ON admin_role_assignment (revoked_by) WHERE revoked_by IS NOT NULL;
CREATE INDEX idx_admin_role_assignment_deviation_entry ON admin_role_assignment (deviation_register_entry_id) WHERE deviation_register_entry_id IS NOT NULL;
CREATE INDEX idx_admin_deviation_log_reviewed_by  ON admin_deviation_log (reviewed_by) WHERE reviewed_by IS NOT NULL;
CREATE INDEX idx_admin_sod_policy_published_by    ON admin_sod_policy (published_by);
CREATE INDEX idx_comp_aml_screening_adjudicated_by ON comp_aml_screening (adjudicated_by) WHERE adjudicated_by IS NOT NULL;
CREATE INDEX idx_comp_sar_case_raised_by          ON comp_sar_case (raised_by);
CREATE INDEX idx_comp_spot_check_completed_by     ON comp_spot_check (completed_by);
CREATE INDEX idx_audit_account_proposed_by        ON audit_account (proposed_by);
CREATE INDEX idx_audit_account_approved_by        ON audit_account (approved_by) WHERE approved_by IS NOT NULL;

-- --- natural-key lookup indexes (NON-unique: PAN/GSTIN/CIN uniqueness is a business
--     decision deferred to the V2 decision pass — proprietorships may share a PAN) --------
CREATE INDEX idx_inv_account_pan        ON inv_account (pan) WHERE pan IS NOT NULL;  -- PAN non-unique: proprietors may share a PAN
-- [AUDIT-FIX DP] GSTIN/CIN are per-entity government IDs: UNIQUE blocks duplicate company onboarding.
CREATE UNIQUE INDEX uidx_sup_account_gstin     ON sup_account (gstin) WHERE gstin IS NOT NULL;
CREATE UNIQUE INDEX uidx_sup_account_cin       ON sup_account (cin) WHERE cin IS NOT NULL;
CREATE UNIQUE INDEX uidx_buyer_account_gstin   ON buyer_account (gstin) WHERE gstin IS NOT NULL;
CREATE UNIQUE INDEX uidx_buyer_account_mca_cin ON buyer_account (mca_cin) WHERE mca_cin IS NOT NULL;

-- =============================================================================
-- AUDIT-FIX DECISION PASS (DL-BE-007, 2026-06-13)
-- Spec-mandated record-level maker-checker (C4) made declarative for Suspend /
-- Blacklist / KYC-approval / auditor-approval, plus approver MFA assertion tokens.
-- Columns added via ALTER (admin_user already created above); maker<>checker and
-- action-requires-two-person CHECKs; FKs to admin_user.
-- =============================================================================

-- sup_account: Suspend AND Blacklist each require a maker-checker pair (C4, B3:733).
ALTER TABLE sup_account
    ADD COLUMN suspend_maker_id                  UUID,
    ADD COLUMN suspend_checker_id                UUID,
    ADD COLUMN suspend_checker_mfa_assertion_id  TEXT,
    ADD COLUMN blacklist_maker_id                UUID,
    ADD COLUMN blacklist_checker_id              UUID,
    ADD COLUMN blacklist_checker_mfa_assertion_id TEXT;
ALTER TABLE sup_account
    ADD CONSTRAINT sup_account_suspend_maker_ne_checker
        CHECK (suspend_checker_id IS NULL OR suspend_maker_id <> suspend_checker_id),
    ADD CONSTRAINT sup_account_blacklist_maker_ne_checker
        CHECK (blacklist_checker_id IS NULL OR blacklist_maker_id <> blacklist_checker_id),
    ADD CONSTRAINT sup_account_suspend_makerchecker_required
        CHECK (suspended_at IS NULL
               OR (suspend_maker_id IS NOT NULL AND suspend_checker_id IS NOT NULL
                   AND suspend_checker_mfa_assertion_id IS NOT NULL)),
    ADD CONSTRAINT sup_account_blacklist_makerchecker_required
        CHECK (blacklisted_at IS NULL
               OR (blacklist_maker_id IS NOT NULL AND blacklist_checker_id IS NOT NULL
                   AND blacklist_checker_mfa_assertion_id IS NOT NULL)),
    ADD CONSTRAINT sup_account_suspend_maker_fk    FOREIGN KEY (suspend_maker_id)    REFERENCES admin_user (admin_user_id),
    ADD CONSTRAINT sup_account_suspend_checker_fk  FOREIGN KEY (suspend_checker_id)  REFERENCES admin_user (admin_user_id),
    ADD CONSTRAINT sup_account_blacklist_maker_fk   FOREIGN KEY (blacklist_maker_id)   REFERENCES admin_user (admin_user_id),
    ADD CONSTRAINT sup_account_blacklist_checker_fk FOREIGN KEY (blacklist_checker_id) REFERENCES admin_user (admin_user_id);
CREATE INDEX idx_sup_account_suspend_checker   ON sup_account (suspend_checker_id)   WHERE suspend_checker_id IS NOT NULL;
CREATE INDEX idx_sup_account_blacklist_checker ON sup_account (blacklist_checker_id) WHERE blacklist_checker_id IS NOT NULL;

-- buyer_account: Suspend requires a maker-checker pair (C4, B3:819).
ALTER TABLE buyer_account
    ADD COLUMN suspend_maker_id                 UUID,
    ADD COLUMN suspend_checker_id               UUID,
    ADD COLUMN suspend_checker_mfa_assertion_id TEXT;
ALTER TABLE buyer_account
    ADD CONSTRAINT buyer_account_suspend_maker_ne_checker
        CHECK (suspend_checker_id IS NULL OR suspend_maker_id <> suspend_checker_id),
    ADD CONSTRAINT buyer_account_suspend_makerchecker_required
        CHECK (suspended_at IS NULL
               OR (suspend_maker_id IS NOT NULL AND suspend_checker_id IS NOT NULL
                   AND suspend_checker_mfa_assertion_id IS NOT NULL)),
    ADD CONSTRAINT buyer_account_suspend_maker_fk   FOREIGN KEY (suspend_maker_id)   REFERENCES admin_user (admin_user_id),
    ADD CONSTRAINT buyer_account_suspend_checker_fk FOREIGN KEY (suspend_checker_id) REFERENCES admin_user (admin_user_id);
CREATE INDEX idx_buyer_account_suspend_checker ON buyer_account (suspend_checker_id) WHERE suspend_checker_id IS NOT NULL;

-- comp_kyc_file: approver must differ from supplier-side submitter (KF.2, C4) + approver MFA.
ALTER TABLE comp_kyc_file
    ADD COLUMN submitted_by              UUID,
    ADD COLUMN approver_mfa_assertion_id TEXT;
ALTER TABLE comp_kyc_file
    ADD CONSTRAINT comp_kyc_file_maker_ne_checker
        CHECK (approver_id IS NULL OR submitted_by IS NULL OR approver_id <> submitted_by),
    ADD CONSTRAINT comp_kyc_file_approver_mfa_chk
        CHECK (approver_id IS NULL OR approver_mfa_assertion_id IS NOT NULL),
    ADD CONSTRAINT comp_kyc_file_submitted_by_fk
        FOREIGN KEY (submitted_by) REFERENCES admin_user (admin_user_id);
CREATE INDEX idx_comp_kyc_file_submitted_by ON comp_kyc_file (submitted_by) WHERE submitted_by IS NOT NULL;

-- audit_account: record the approver's MFA assertion (C7); proposed_by<>approved_by already enforced.
ALTER TABLE audit_account
    ADD COLUMN approved_mfa_assertion_id TEXT;
ALTER TABLE audit_account
    ADD CONSTRAINT audit_account_approver_mfa_chk
        CHECK (approved_by IS NULL OR approved_mfa_assertion_id IS NOT NULL);
