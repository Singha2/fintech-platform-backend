...
7. **BE-18 · Phase B investor login + self-commit** — passwordless invite→email+OTP investor login **+** investor self-commit (the `CommandGateway` non-admin-actor authz change) + audit of denied cross-tenant reads. Unblocks **real** investor login (M10-D shipped dev-password-only) and investor-initiated payments. **🟡 DoR-green + `/plan` done, implementation pending** (`/specify`+`/clarify`+`/plan` done; `/tasks` next). Assessment: **mostly additive, no architectural break** (the gateway/session/audit substrates already accommodate a non-admin actor — no new tables). **Spec:** [`modules/M11-B-investor-login-selfcommit.md`](modules/M11-B-investor-login-selfcommit.md) · **DoR decisions:** `DL-BE-088` · **Brief:** [`BE18_INVESTOR_LOGIN_SELFCOMMIT_BRIEF.md`](BE18_INVESTOR_LOGIN_SELFCOMMIT_BRIEF.md); rationale in `DL-BE-084` (§Phase B).
...
|---|---|---|---|
| **DF-1** | **Real investor login + self-commit** (passwordless). M10-D ships **dev-password login only**; real investors can't self-login/pay yet. | = **BE-18** above (Track B #7) · brief [`BE18_INVESTOR_LOGIN_SELFCOMMIT_BRIEF.md`](BE18_INVESTOR_LOGIN_SELFCOMMIT_BRIEF.md) · `DL-BE-084`, `M10-D §DoR-1` | Before any pilot with real investors logging in / paying. |
