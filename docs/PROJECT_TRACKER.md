...
| **S15** Buyer portal | ack-user OTP login + buyer reads + self-ack (BE-15) | 🟡 | ✅ | ❌ | **BE-15 DoR + `/plan` done, implementation pending.** Mostly BE-18 again for the `acknowledgment_user` kind; **one non-additive edit** (verified one-line) — makes the live S5 `recordBuyerAck` actor-kind-aware (de-couple `captured_by`). Edges resolved: self-ack requires an outstanding request (E2), no double-ack (E3); payment-instruction metadata-only (confirm-PI stores no bank details — E1). Spec: [`modules/M11-C-buyer-portal-ack.md`](modules/M11-C-buyer-portal-ack.md) · `DL-BE-090` | BE-15 / WS-2 |
...
5. **Deferred screens** — S9 (after M17), S15 (**BE-15 / WS-2·M2 — DoR + `/plan` done, spec `M11-C-buyer-portal-ack.md`, `DL-BE-090`; `/tasks` next**; one non-additive edit = the `recordBuyerAck` actor-kind branch).
