...
| **🟢 Live wiring (the bridge)** | ✅ **Auth + reads + admin/deal-flow writes live & E2E-verified** — `src/api/` client + envelope + `ApiError` + full service layer + `AuthContext` + Vite proxy. **S1 login**; **reads** for S2–S8/S11/S12/S14; **write chains** onboarding (S3/S4/S8 + **S10 investor**) + deal spine **S5 go-live → S12 subscribe → S6 disburse → S7 mature/distribute** (harnesses `scripts/e2e/*`, DL-BE-086/087 seed helper + two-ops). Remaining: investor self-service — S13 portfolio, real investor login (BE-18); S9 (M17), S15 (WS-2). | mock `docs/UI_WORKORDER.md` |

**The admin + deal-flow surface — including investor onboarding (S10) — is done and verified.** The remaining
critical path is the **investor self-service side**: S13 portfolio and real investor login/self-commit (**BE-18**),
then the two await-milestone screens S9 (M17) and S15 (WS-2).
