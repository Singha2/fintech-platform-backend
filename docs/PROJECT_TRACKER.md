...
| **DF-5** | **No `POST /auth/logout` endpoint** — the UI "Log out" clears the bearer client-side only; the token stays valid server-side until expiry (a real gap on shared/pilot machines). `SessionService.revokeSession` + the `auth.Session.Revoked` audit **already exist** — only the controller is missing (~6 lines on `SessionController`). | `SessionController` (backend) · mock already wired best-effort | **Backend follow-up (open)** — brief: [`LOGOUT_ENDPOINT_BRIEF.md`](LOGOUT_ENDPOINT_BRIEF.md). Expose `POST /auth/logout` → `revokeSession(session.sessionId())` → 204 (idempotent). Add `DL-BE-089`. UI calls it best-effort today, so it activates on ship. |
