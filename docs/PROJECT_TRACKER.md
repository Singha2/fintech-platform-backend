...
| **DF-3** | **`DevDataSeeder` is all-or-nothing empty-guarded** — it seeds only when `admin_user` is empty, so seed admins added later (e.g. `ops2@dev.local`, DL-BE-086) never materialize on a pre-existing dev DB. The running dev DB had 6 admins → seeder skipped → verifying DOC.3 (S5 go-live) needed a manual `ops2` insert. | `DevDataSeeder.run` / `seedAdmin` (backend) | **Backend follow-up (open).** Proposed: extract `ensureAdmin(email,name,role)` that returns the existing `admin_user_id` when a row with that `email::citext` exists else calls `seedAdmin`; call it for all seven accounts on every dev boot (unconditionally); keep the counterparty block guarded on `sup_account` emptiness. Idempotent, so the manually-inserted `ops2` is adopted, not duplicated. Add `DL-BE-087`. |
| **DF-4** | **S5 has no invoice-document upload UI** — `document_completeness` (BC16 / DOC.3) needs an attached invoice PDF, but the screen can't upload one, so go-live can't be completed from the UI in live mode (backend path works). | mock `S5.jsx` + a `documents` service (`src/api/services/`) | **Now** — add the two-phase upload (initiate → PUT content → finalize → attach), then record `document_completeness` as a second Ops (DOC.3). *(→ implemented: mock commit)* |
